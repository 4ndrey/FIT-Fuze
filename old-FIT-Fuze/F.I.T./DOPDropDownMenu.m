...
//    CGFloat separatorLineInterval = self.frame.size.width / _numOfMenu;
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        NSRange stringRange = {0, MIN([titleString length], 25)};
        stringRange = [titleString rangeOfComposedCharacterSequencesForRange:stringRange];
        NSString *shortString = [titleString substringWithRange:stringRange];
        
        CATextLayer *title = [self createTextLayerWithNSString:[shortString stringByAppendingString:@"..."] withColor:self.textColor andPosition:titlePosition];
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    layer.string = string;
    layer.fontSize = 18.0;
    layer.font = (__bridge CFTypeRef)([UIFont systemFontOfSize:18 weight:UIFontWeightLight]);
...
{
    NSDictionary *dic = @{NSFontAttributeName: [UIFont systemFontOfSize:18 weight:UIFontWeightLight]};
...
- (void)confiMenuWithSelectRow:(NSInteger)row {
    ...
    NSString *titleString = [self.dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:self.currentSelectedMenudIndex row:row]];
    NSRange stringRange = {0, MIN([titleString length], 25)};
    stringRange = [titleString rangeOfComposedCharacterSequencesForRange:stringRange];
    NSString *shortString = [titleString substringWithRange:stringRange];
    title.string = shortString;
